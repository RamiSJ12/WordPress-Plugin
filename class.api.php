<?php
class create_movie_post extends create_movie_post {
    /**************************** CURL SETTINGS *********************************************/
    ...
    function api_exist(){
        global $wpdb;
        $value= "";
        $result = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'options WHERE option_name="shortcode-movie-api"');
        if(count($result)>0){
            foreach ($result as $r){
    ...
    function ExtentionCheck($ex=""){
        global $wpdb;
        $value="none";
        $result = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'options WHERE option_name LIKE "shortcode-movie-ex-'.$ex.'%"');
        if(count($result)>0){
            $value = array();
            foreach ($result as $r){
                $def = $ex == "" ? str_replace('shortcode-movie-ex-','',$r->option_name) : $ex;
    ...
    function check_api($api){
		...
		if((isset($json->member) == "active") && ($forbidden=="ok")){
			...
			if($check!="") {
				...
				$wpdb->query('DELETE FROM '.$wpdb->prefix.'options WHERE option_name  LIKE "shortcode-movie%"');
			}
			$wpdb->insert(
				$wpdb->prefix.'options',
				array(
					'option_name'    => "shortcode-movie-api",
					'option_value'   => $api,
				)
			);
			...
			foreach ($exList as $ext){
				if(isset($json->$ext)){
					$wpdb->insert(
						$wpdb->prefix . 'options',
						array(
							'option_name' => "shortcode-movie-ex-".$ext,
			...
			if(isset($_SESSION['movie_secret_key'])) {
				unset($_SESSION['movie_secret_key']);
			}
			$_SESSION['movie_secret_key'] = $json->secret;
	...
	function get_secret_key($api){
		if(isset($_SESSION['movie_secret_key'])){
			$key =  $_SESSION['movie_secret_key'];
		} else{
			$key = $this->curl3($this->baseUrl().$api."/secret");
			@$_SESSION['movie_secret_key'] = $key;
		}
		return $key;
	}
